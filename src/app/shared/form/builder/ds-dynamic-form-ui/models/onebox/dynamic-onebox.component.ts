...
import { DynamicFormLayoutService, DynamicFormValidationService, DynamicFormControlEventType,DynamicFormControlEvent } from '@ng-dynamic-forms/core';
import {
  catchError,
  debounceTime,
...
export class DsDynamicOneboxComponent extends DsDynamicVocabularyComponent implements OnInit {
  ...
  @ViewChild(DsDynamicFormControlContainerComponent) formControlContainer: DsDynamicFormControlContainerComponent;
  ...
  inputValue: any;
  preloadLevel: number;
  collegeName: string; //kware-edit mohamed
      ...
      switchMap((term) => {
        if (term === '' || term.length < this.model.minChars) {
          ...
          return this.vocabularyService.getVocabularyEntriesByValue(
            term,
            false,
            this.model?.vocabularyOptions,  //kware-edit mohamed
            this.pageInfo).pipe(
  ...
  ngOnInit() {
   ...
   if (this.model.id === 'dc_relation_college'){
    this.sharedVariableService.currentCollege.subscribe(college=>{
      if (this.collegeName !== college){
        this.collegeName = college;
        if (this.model.value && this.collegeName ) {
          this.setCurrentValue(this.collegeName ? this.collegeName : null, true);
        }
      }
    });
 ...
 //kware-edit mohamed
  ...
  openLookup() {
    console.log('test12');
    this.formControlContainer.openLookup();
    }
